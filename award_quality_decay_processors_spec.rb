...
RSpec.describe AwardQualityDecayProcessors do
  it 'will return the proper processor for each award type' do
    [['Blue First', BlueFirst],
     ...
     ['Blue Star', BlueStar]].each do |values|
      processor = retrieve_processor(Award.new(values.first, 10, 100))
  ...
  it 'will raise an error if it cannot find the proper processor' do
    ...
    expect {
      retrieve_processor(award)
    }.to raise_error(AwardQualityDecayProcessors::UnknownProcessor)
  end
