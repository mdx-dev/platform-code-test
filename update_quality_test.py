...
@pytest.mark.parametrize(
    "initial_expires_in,initial_quality,expected_quality",
    [
        (5, 10, 9),
        (0, 10, 8),
        (-10, 10, 8),
...
def test_normal_award(initial_expires_in, initial_quality, expected_quality):
    award = Award(
        name="NORMAL ITEM", expires_in=initial_expires_in, quality=initial_quality
    )
...
def test_blue_first(initial_expires_in, initial_quality, expected_quality):
    award = Award(
        name="Blue First", expires_in=initial_expires_in, quality=initial_quality
    )
    update_quality([award])
    assert award.expires_in == initial_expires_in - 1
    assert award.quality == expected_quality
...
def test_blue_distinction(initial_expires_in, initial_quality, expected_quality):
    award = Award(
...
def test_blue_compare(initial_expires_in, initial_quality, expected_quality):
    award = Award(
        name="Blue Compare", expires_in=initial_expires_in, quality=initial_quality
    )
    update_quality([award])
    assert award.expires_in == initial_expires_in - 1
    assert award.quality == expected_quality
...
def test_blue_star(initial_expires_in, initial_quality, expected_quality):
    award = Award(
        name="Blue Star", expires_in=initial_expires_in, quality=initial_quality
    )
    update_quality([award])
    assert award.expires_in == initial_expires_in - 1
    assert award.quality == expected_quality
